<template>
  <div class="common-layout">
    <el-container>
      <el-header>
        <div class="header-bar">
          <div class="title">玉米产量预测区域版（GNAH）</div>
          <div class="actions">
                  ...
                  <div class="param-card">
                    ...
                    <div class="param-description">
                      ...
                      <el-form :model="config" label-width="100px">
                        <el-form-item label="模型文件d1">
                          <el-select v-model="config.current_model_d1" style="width: 100%;" placeholder="请选择模型文件d1（.enc）">
                            <el-option 
                              v-for="model in availableModelsD1" 
                              :key="model" 
                              :label="model" 
                              :value="model" 
                            />
                        ...
                        <el-form-item label="模型文件d2">
                          <el-select v-model="config.current_model_d2" style="width: 100%;" placeholder="请选择模型文件d2（.joblib）">
                            <el-option 
                              v-for="model in availableModelsD2" 
                              :key="model" 
                              :label="model" 
                              :value="model" 
                            />
                ...
                <div class="right-column">
                  <div class="param-card">
                    ...
                    <div class="upload-description">
                        上传ERA5气象数据、MODIS反射率数据和MODIS光合有效辐射数据，这些文件包含用于玉米产量预测区域版（GNAH）预测的所有输入数据
                    </div>
                  ...
                  <div class="parameters-section">
                    <div class="parameters-grid">
                      <div class="parameter-item">
                        <label>cc4:</label>
                        <el-input-number v-model.number="gnahCoefficients.cc4" :min="0" :max="100" :step="0.01" :precision="3" size="small" style="width:150px" />
                        </div>
                      <div class="parameter-item">
                        <label>factor_yield1:</label>
                        <el-input-number v-model.number="gnahCoefficients.factor_yield1" :min="0" :max="1" :step="0.001" :precision="4" size="small" style="width:150px" />
                        </div>
                      <div class="parameter-item">
                        <label>factor_yield2:</label>
                        <el-input-number v-model.number="gnahCoefficients.factor_yield2" :min="0" :max="1000" :step="0.1" :precision="3" size="small" style="width:150px" />
                        </div>
                        </div>
    ...
    <!-- 数据文件上传对话框 -->
    <el-dialog v-model="showDataUpload" title="玉米产量预测区域版（GNAH） - 数据文件上传" width="90%" :close-on-click-modal="false">
      <div class="maize-estimate-upload">
        <div class="upload-header">
          ...
          >
            <template #default>
              请上传以下文件：ERA5气象数据（.tif）、MODIS反射率数据（.tif）、MODIS光合有效辐射数据（.tif）。
        ...
        <!-- 三个数据输入区域 -->
        <div class="upload-data-grid" style="grid-template-columns: 1fr 1fr 1fr;">
          <!-- ERA5气象数据 -->
          <div class="upload-data-item">
            <div class="data-input-header">
              <div class="data-input-title">ERA5气象数据（ERA5）</div>
          ...
          <div class="upload-data-item">
            <div class="data-input-header">
              <div class="data-input-title">MODIS反射率数据（MODIS_Ref）</div>
              <div class="data-input-desc">包含所有.tif文件</div>
            </div>
            <div 
              class="multiple-upload-zone"
              :class="{ 'drag-over': modisRefDragOver }"
              @dragover.prevent="handleModisRefDragOver"
              @dragleave.prevent="handleModisRefDragLeave"
              @drop.prevent="handleModisRefDrop"
            >
              <div class="upload-zone-content">
                <i class="el-icon-upload"></i>
                <p>拖拽多个.tif文件到此处</p>
                <p class="upload-hint">或</p>
                <el-button @click="selectModisRefFolder" type="primary" size="default">选择文件夹</el-button>
                <input ref="modisRefInput" type="file" webkitdirectory directory multiple @change="handleModisRefSelect" style="display: none" accept=".tif" />
              </div>
            </div>
            <div v-if="modisRefFiles.length > 0" class="file-list">
              <div v-for="(file, index) in modisRefFiles" :key="index" class="file-item">
                <i class="el-icon-document"></i><span>{{ file.name }}</span>
              </div>
            </div>
            </div>
          ...
          <div class="upload-data-item">
            <div class="data-input-header">
              <div class="data-input-title">MODIS光合有效辐射数据（MODIS_PAR）</div>
              <div class="data-input-desc">包含所有.tif文件</div>
              </div>
            <div 
              class="multiple-upload-zone"
              :class="{ 'drag-over': modisParDragOver }"
              @dragover.prevent="handleModisParDragOver"
              @dragleave.prevent="handleModisParDragLeave"
              @drop.prevent="handleModisParDrop"
            >
              <div class="upload-zone-content">
                <i class="el-icon-upload"></i>
                <p>拖拽多个.tif文件到此处</p>
                <p class="upload-hint">或</p>
                <el-button @click="selectModisParFolder" type="primary" size="default">选择文件夹</el-button>
                <input ref="modisParInput" type="file" webkitdirectory directory multiple @change="handleModisParSelect" style="display: none" accept=".tif" />
              </div>
            </div>
            <div v-if="modisParFiles.length > 0" class="file-list">
              <div v-for="(file, index) in modisParFiles" :key="index" class="file-item">
                <i class="el-icon-document"></i><span>{{ file.name }}</span>
              </div>
            </div>
          </div>
    ...
    <!-- 任务历史对话框 -->
    <el-dialog v-model="showTaskHistory" title="玉米产量预测区域版（GNAH）任务历史" width="95%" :close-on-click-modal="false">
      <div class="task-history">
        ...
        <el-table :data="taskHistory" v-loading="taskHistoryLoading" stripe style="width: 100%">
          ...
          <el-table-column label="算法" width="200">
            <template #default="{ row }">
              <span class="algorithm-name">玉米产量预测区域版（GNAH）</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="预览结果" width="200" align="center">
            <template #default="{ row }">
              <div v-if="row.status === 'completed'" class="preview-container">
        ...
        <div v-if="taskDetailData" class="task-detail-content">
          ...
          <div class="task-info-section">
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                <span class="info-label">模型文件d1:</span>
                <span class="info-value">
                  {{ taskDetailData.model_files_used?.model_d1 || '未记录' }}
                </span>
              </div>
              <div class="info-item">
                <span class="info-label">模型文件d2:</span>
                <span class="info-value">
                  {{ taskDetailData.model_files_used?.model_d2 || '未记录' }}
                </span>
              </div>
...
const modisRefDragOver = ref(false)
const modisParDragOver = ref(false)

// 文件列表
const era5Files = ref([])
const modisRefFiles = ref([])
const modisParFiles = ref([])

// GNAH模型配置
const config = ref({
  model_d1: 'Model_d1.enc',
  model_d2: 'Model_d2.joblib',
  current_model_d1: 'Model_d1.enc',
  current_model_d2: 'Model_d2.joblib'
})

// 可用的模型文件列表
const availableModelsD1 = ref(['Model_d1.enc'])
const availableModelsD2 = ref(['Model_d2.joblib'])

// GNAH参数配置（保留用于向后兼容，但不再显示在界面上）
const gnahParams = ref({
  output_prefix: 'GNAH_Yield_Task1'
})

// GNAH模型系数配置
const gnahCoefficients = ref({
  cc4: 5.18,
  factor_yield1: 0.027,
...
const modisRefInput = ref(null)
const modisParInput = ref(null)
...
// MODIS反射率文件处理
const selectModisRefFolder = () => { if (modisRefInput.value) modisRefInput.value.click() }
const handleModisRefSelect = (event) => {
  const files = Array.from(event.target.files || []).filter(f => f.name.toLowerCase().endsWith('.tif'))
  if (files.length === 0) { ElMessage.warning('所选文件夹中没有找到.tif文件'); return }
  modisRefFiles.value = files
  ElMessage.success(`已选择 ${files.length} 个MODIS反射率文件`)
  if (modisRefInput.value) modisRefInput.value.value = ''
}
const handleModisRefDragOver = () => { modisRefDragOver.value = true }
const handleModisRefDragLeave = () => { modisRefDragOver.value = false }
const handleModisRefDrop = (event) => {
  modisRefDragOver.value = false
  const files = Array.from(event.dataTransfer.files).filter(f => f.name.toLowerCase().endsWith('.tif'))
  if (files.length === 0) { ElMessage.warning('请拖拽.tif格式的文件'); return }
  modisRefFiles.value = files
  ElMessage.success(`已选择 ${files.length} 个MODIS反射率文件`)
...
// MODIS光合有效辐射文件处理
const selectModisParFolder = () => { if (modisParInput.value) modisParInput.value.click() }
const handleModisParSelect = (event) => {
  const files = Array.from(event.target.files || []).filter(f => f.name.toLowerCase().endsWith('.tif'))
  if (files.length === 0) { ElMessage.warning('所选文件夹中没有找到.tif文件'); return }
  modisParFiles.value = files
  ElMessage.success(`已选择 ${files.length} 个MODIS光合有效辐射文件`)
  if (modisParInput.value) modisParInput.value.value = ''
  }
const handleModisParDragOver = () => { modisParDragOver.value = true }
const handleModisParDragLeave = () => { modisParDragOver.value = false }
const handleModisParDrop = (event) => {
  modisParDragOver.value = false
  const files = Array.from(event.dataTransfer.files).filter(f => f.name.toLowerCase().endsWith('.tif'))
  if (files.length === 0) { ElMessage.warning('请拖拽.tif格式的文件'); return }
  modisParFiles.value = files
  ElMessage.success(`已选择 ${files.length} 个MODIS光合有效辐射文件`)
...
const handleUploadConfirm = async () => {
  ...
  if (modisRefFiles.value.length === 0) {
    ElMessage.error('请上传MODIS反射率数据文件')
    return
  }
  
  if (modisParFiles.value.length === 0) {
    ElMessage.error('请上传MODIS光合有效辐射数据文件')
    return
  }
...
const runPrediction = async () => {
  try {
    ...
    // 上传MODIS反射率文件
    progressText.value = `正在上传 ${modisRefFiles.value.length} 个MODIS反射率文件...`
    modisRefFiles.value.forEach(file => {
      formData.append('modis_ref_files', file)
    })
    
    // 上传MODIS光合有效辐射文件
    progressText.value = `正在上传 ${modisParFiles.value.length} 个MODIS光合有效辐射文件...`
    modisParFiles.value.forEach(file => {
      formData.append('modis_par_files', file)
      })
    
    // 添加输出前缀
    if (gnahParams.value.output_prefix) {
      formData.append('output_prefix', gnahParams.value.output_prefix)
    }
    
    // 更新进度提示
    progressText.value = '文件上传完成，正在运行玉米产量预测区域版（GNAH）预测算法，请稍候...（预计需要较长时间）'
    
    // 开始预测请求
    const res = await runGNAH(formData, abortController.value)
...
const cancelPrediction = async () => {
  ...
  try {
    ...
    if (currentTaskId.value) {
      try {
        await cancelGNAHTask(currentTaskId.value)
...
const loadTaskHistory = async () => {
  try {
    ...
    const res = await getGNAHTasks()
    if (res && res.data) {
      taskHistory.value = res.data.tasks || []
      
      // 预加载已完成任务的预览图片
      for (const task of taskHistory.value) {
        if (task.status === 'completed' && !previewCache.value[task.task_id]) {
          // 异步加载，不阻塞界面
...
const loadTaskDetail = async (taskId) => {
  try {
    ...
    const res = await getGNAHTask(taskId)
    if (res && res.data) {
...
const deleteTask = async (task) => {
  try {
    ...
    await deleteGNAHTask(task.task_id)
...
const downloadResult = async (format) => {
  ...
  try {
    const res = await downloadGNAHResult(output.value.task_id, format)
    const blob = new Blob([res.data])
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    const ext = format === 'tif' ? 'tif' : format === 'png' ? 'png' : 'json'
    link.download = `gnah_result.${ext}`
    link.click()
...
const downloadTaskResult = async (task) => {
  try {
    // 默认下载TIF格式
    const res = await downloadGNAHResult(task.task_id, 'tif')
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `gnah_result_${task.task_id}.tif`
...
const loadPreviewImage = async (taskId) => {
  ...
  try {
    const res = await downloadGNAHResult(taskId, 'png')
...
const downloadPngResult = async (task) => {
  try {
    const res = await downloadGNAHResult(task.task_id, 'png')
    ...
    const link = document.createElement('a')
    link.href = url
    link.download = `gnah_result_${task.task_id}.png`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    window.URL.revokeObjectURL(url)
...
// 获取GNAH参数
const fetchGNAHParams = async () => {
  try {
    const res = await getGNAHParams()
    if (res && res.data) {
      // 更新模型配置
      if (res.data.config) {
      ...
      // 更新可用模型列表
      if (res.data.available_models_d1) {
        availableModelsD1.value = res.data.available_models_d1
      }
      if (res.data.available_models_d2) {
        availableModelsD2.value = res.data.available_models_d2
      }
      // 更新算法参数（保留用于向后兼容）
      if (res.data.params) {
        gnahParams.value = { ...gnahParams.value, ...res.data.params }
      }
      // 更新模型系数
      if (res.data.coefficients) {
        gnahCoefficients.value = { ...gnahCoefficients.value, ...res.data.coefficients }
      }
    }
  } catch (e) {
...
const saveConfig = async () => {
  try {
    await updateGNAHParams({
      config: {
        current_model_d1: config.value.current_model_d1,
        current_model_d2: config.value.current_model_d2
      },
      params: gnahParams.value  // 保留算法参数
    })
    ElMessage.success('配置已保存')
  } catch (e) {
    const msg = e?.response?.data?.detail || e?.response?.data?.error || '配置保存失败'
    ElMessage.error(typeof msg === 'string' ? msg : '配置保存失败')
  }
}
...
// 获取当前参数（保留用于向后兼容，但不再使用）
const getCurrentParams = async () => {
  await getCurrentConfig()
}

// 重置参数为默认值
const resetParamsDefaults = async () => {
  try {
    const res = await getGNAHDefaults()
    if (res && res.data) {
      if (res.data.config) {
      ...
      if (res.data.params) {
        gnahParams.value = { ...res.data.params }
      }
      if (res.data.coefficients) {
        gnahCoefficients.value = { ...res.data.coefficients }
      }
...
const saveCoefficients = async () => {
  try {
    await updateGNAHParams({
      coefficients: {
        cc4: gnahCoefficients.value.cc4,
        factor_yield1: gnahCoefficients.value.factor_yield1,
        factor_yield2: gnahCoefficients.value.factor_yield2
      }
    })
...
const resetCoefficientsDefaults = async () => {
  try {
    ...
    if (res && res.data && res.data.coefficients) {
      gnahCoefficients.value = { ...res.data.coefficients }
      ElMessage.success('模型系数已重置为默认值')
    }
  } catch (e) {
    ElMessage.error('重置模型系数失败')
  }
}
